...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.stride = stride
#         if hook:
#             self.bw_hook = Hook(self.conv1,backward=True)
#             self.fw_hook = Hook(self.conv1)
...
class ResNet_StoDepth_lineardecay(nn.Module):
    ...
    def __init__(self, block, prob_0_L, multFlag, layers, num_classes=1000, zero_init_residual=False,type_=0,dist=False):
        ...
        if dist:
            if type_==0:
                ...
                dni.BasicSynthesizer(
                 output_dim=8192, n_hidden=1, context_dim=None
                 )
             )
            if type_==1:
                self.inplanes = 512
...
#                 self.inplanes = 512
                self.repeatlayer3 = self._make_layer(block, 256, layers[2], stride=2,repeat=True)
                self.backward_interface = dni.BackwardInterface(
                dni.BasicSynthesizer(
                 output_dim=4096, n_hidden=1, context_dim=None
                 )
             )
            if type_==2:
                self.inplanes = 1024
    ...
    def _make_layer(self, block, planes, blocks, stride=1,repeat=False):
        downsample = None
    ...
    def forward(self, x):
        if self.dist:
            if self.type == 0:
                ...
                context = None
                temp_size = x.size()
                x = x.view(x.size(0),-1) #combines batches and channels together
                with dni.synthesizer_context(context):
                    x = self.backward_interface(x) #break graph here
                x = x.view(temp_size)
                return x
            elif self.type == 1:
                if x.size(1) == 512:
...
class Dist_ResNet_MNIST(nn.Module):
# 224 is ImageNet = 224*224*256

    def __init__(self, block, layers, type_ = 0, dist_ = True, num_classes=10):
        self.inplanes = 4
        self.dist = dist_
        super(Dist_ResNet_MNIST, self).__init__()
        self.conv1 = nn.Conv2d(1, 4, kernel_size=3, stride=1, padding=1,
                               bias=False) #3 channels for CIFAR, 1 for MNIST
        ...
        if dist_:
            if type_ == 0:
                ...
                self.layer2 = self._make_layer(block, 8, layers[1], stride=1)
            elif type_ == 1:
                self.inplanes = 32
                self.layer3 = self._make_layer(block, 8, layers[2], stride=1) 
                self.repeatlayer3 = self._make_layer(block,8,layers[2], stride=1,repeat=True)
            elif type_ == 2:
                self.inplanes = 32
#                 self.layer3 = self._make_layer(block, 8, layers[2], stride=1) 
                self.repeatlayer3 = self._make_layer(block,8,layers[2], stride=1,repeat=True)
        ...
        self.fc = nn.Linear(576, num_classes) #576 for mnist - 1024 for CIFARs
#         if dist_:
        self.type = type_
        self.backward_interface = dni.BackwardInterface(
             dni.BasicSynthesizer(
                 output_dim=6272, n_hidden=1, context_dim=None
                 )
             ) #mnist - 6272, cifar - 8192
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
...
#                 x = self.layer3(x)
#                 x = self.repeatlayer3(x)    #comment out for 4 GPUS
                x = self.layer4(x)
                x = self.avgpool(x)
                x = x.view(x.size(0), -1)
                x = self.fc(x)
